...
* Creation Date: 2019-07-01 14:05
* 
* Description: GraceQ/mps2  project. MPI parallel Lanczos implementation unittests.
*/
...
struct TestDistributedLanczos : public testing::Test {
...
void RunTestCentDistributedLanczosSolverCase(
...
TEST_F(TestDistributedLanczos, TestCentDistributedLanczosSolver) {
  ...
  RunTestCentDistributedLanczosSolverCase(
...
void RunTestLendDistributedLanczosSolverCase(
    const std::vector<GQTensor *> &eff_ham,
    GQTensor *init_state,
    const LanczosParams &lanczos_params,
    const int workers) {
...
TEST_F(TestDistributedLanczos, TestLendLanczosSolver) {
  ...
  init_state->Random(QN({QNNameVal("Sz", 0)}));
  LanczosParams lanczos_params(1.0E-9);
  RunTestLendDistributedLanczosSolverCase(
