...
struct TestTwoSiteAlgorithmSpinSystem : public testing::Test {
  ...
  DGQTensor sz = DGQTensor({pb_in, pb_out});
  DGQTensor sp = DGQTensor({pb_in, pb_out});
  DGQTensor sm = DGQTensor({pb_in, pb_out});
...
TEST_F(TestTwoSiteAlgorithmSpinSystem, 1DIsing) {
  auto mpo_gen = MPOGenerator<GQTEN_Double>(N, pb_out, qn0);
  for (long i = 0; i < N-1; ++i) {
    mpo_gen.AddTerm(1, {sz, sz}, {i, i+1});
  }
  auto mpo = mpo_gen.Gen();

  std::vector<DGQTensor *> mps(N);
